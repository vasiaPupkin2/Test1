...
//  Test task
//
//  Created by leanid niadzelin on 03.10.17.
...
class LoginController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, LoginCellDelegate {
    ...
    @objc private func handleLoginInformationChanged() {
        ...
        let isFormValid = loginCell.loginTextField.text?.characters.count ?? 0 > 0 && loginCell.passwordTextField.text?.characters.count ?? 0 > 7
        if isFormValid {
    ...
    @objc func handleSignUpInformationChanged() {
        ...
        let isFormValid = registerCell.loginTextField.text?.characters.count ?? 0 > 0 &&
            registerCell.passwordTextField.text?.characters.count ?? 0 > 7 &&
            registerCell.repeatPasswordField.text?.characters.count ?? 0 > 7 &&
        ...
        API.shared.logingIn(login: login, password: password, completion: { success, response, error in
            if success {
                DispatchQueue.main.async {
                    ...
                    if let mainViewController = rootViewController as? MainNavController {
                        let homeController = HomeController(collectionViewLayout: UICollectionViewFlowLayout())
                        mainViewController.viewControllers = [homeController]
